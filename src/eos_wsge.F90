...
!------------------------------------------------------------------------
  ...
  subroutine eos_wsge_transition_to_single_phase(self, old_primary, old_fluid, &
    ...
    associate (pressure => primary(1), salt => primary(3), &
    ...
    subroutine transition_to_single_phase_interpolated(xi)
      ...
      associate (pressure => primary(1), temperature => primary(2), &
        ...
        if (new_water_region == 1) then
           ...
           call self%thermo%saturation%temperature(interpolated_brine_pressure, &
                temperature, err)
        end if
...
!------------------------------------------------------------------------
  ...
  subroutine eos_wsge_phase_properties(self, primary, rock, fluid, err)
    ...
    PetscReal :: brine_pressure, energy_solution
          ...
          do p = 1, self%num_mobile_phases
             associate(phase => fluid%phase(p), region => self%thermo%region(p)%ptr)
               ...
               if (btest(phases, p - 1)) then
                  ...
                  if (p == 1) then
                     brine_pressure = fluid%pressure
                     call brine_properties(brine_pressure, fluid%temperature, &
                     ...
                     call region%properties([brine_pressure, fluid%temperature], &
                          properties, err)
...
!------------------------------------------------------------------------
  ...
  subroutine eos_wsge_saturation_difference(x, context, f, err)
  ...
  end subroutine eos_wsge_saturation_difference
