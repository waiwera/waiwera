...
!------------------------------------------------------------------------
  ...
  subroutine eos_wge_phase_properties(self, primary, rock, fluid, err)
    ...
    PetscErrorCode, intent(out) :: err !! Error code
    ! Locals:
    PetscInt :: region, p, phases, rindex
    PetscReal :: sl, xg
       ...
       do p = 1, self%num_phases
          associate(phase => fluid%phase(p))
            ...
            if (btest(phases, p - 1)) then
               ...
               rindex = self%region_index(p, region)
               associate (regionp => self%thermo%region(rindex)%ptr, liquid => (p == 1))
...
!------------------------------------------------------------------------
  ...
  subroutine eos_wge_saturation_difference(x, context, f, err)
  ...
  end subroutine eos_wge_saturation_difference
