...
!------------------------------------------------------------------------
  ...
  subroutine test_eos_we_bdy_consistency(test)

    ! Test eos_we boundary consistency
    ...
    if (rank == 0) then
       ...
       associate (P1 => primary1(1), T1 => primary1(2), &
         ...
         T1 = 200._dp
         call thermo%saturation%pressure(T1, P1, err)
         P2 = P1
         Sv2 = 0._dp
       end associate
  ...
  end subroutine test_eos_we_bdy_consistency
