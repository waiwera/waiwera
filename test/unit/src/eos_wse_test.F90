...
#include <petsc/finclude/petsc.h>
  ...
  public :: test_eos_wse_bdy_consistency
...
!------------------------------------------------------------------------
  ...
  subroutine test_eos_wse_transition(test)
    ...
    if (rank == 0) then
       ...
       fluid%region = old_fluid%region
       expected_region = 4
       expected_primary = [85.621455812056474e5_dp, 1._dp - small, 4.2429116241129744e-2_dp]
       expected_transition = PETSC_TRUE
       expected_err = 0
...
!------------------------------------------------------------------------
  ...
  subroutine test_eos_wse_bdy_consistency(test)

    ! Test eos_wse boundary consistency
    ...
    if (rank == 0) then
       ...
       associate (P1 => primary1(1), T1 => primary1(2), &
            Xs1 => primary1(3), P2 => primary2(1), &
            Sv2 => primary2(2), Xs2 => primary2(3))
         T1 = 240._dp
         Xs1 = 0.1_dp
         call eos%saturation_pressure(primary1, 1, P1, err)
         P2 = P1
         Sv2 = 0._dp
         Xs2 = Xs1
       end associate
  ...
  end subroutine test_eos_wse_bdy_consistency
