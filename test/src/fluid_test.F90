...
!------------------------------------------------------------------------
  ...
  subroutine test_fluid
    ...
    if (mpi%rank == mpi%output_rank) then
       ...
       allocate(fluid_data(offset + fluid%dof()))
       do i = 1, size(fluid_data)
          fluid_data(i) = dble(i)
       end do
       call fluid%assign(fluid_data, offset)

       call assert_equals(fluid_data(offset+1), fluid%pressure, tol, "pressure")
       call assert_equals(fluid_data(offset+2), fluid%temperature, tol, "temperature")
