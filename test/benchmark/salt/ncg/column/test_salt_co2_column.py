...
AUTOUGH2_FIELDMAP = {
    ...
    'Liquid CO2 mass fraction': 'NCG liquid m'
}
...
WAIWERA_FIELDMAP = {
    ...
    'Liquid CO2 mass fraction': 'fluid_liquid_CO2_mass_fraction'
}
...
test_fields = ["Pressure", "Temperature", "Liquid saturation", "Vapour saturation",
               "Solid saturation", "Liquid salt mass fraction", "Liquid CO2 mass fraction"]
...
salt_co2_column_test = SciBenchmarkTest(model_name + "_test", nproc = args.np)
salt_co2_column_test.description = """Vertical column salt CO2 test
"""
...
salt_co2_column_test.mSuite.addRun(model_run, run_name)

salt_co2_column_test.setupEmptyTestCompsList()
...
salt_co2_column_test.addTestComp(run_index, "AUTOUGH2",
...
testResult, mResults = salt_co2_column_test.runTest(jrunner, createReports = True)
...
for field_name in plot_fields:
    ...
    result = salt_co2_column_test.mSuite.resultsList[run_index]
    ...
    img_filename = os.path.join(salt_co2_column_test.mSuite.runs[run_index].basePath,
                                salt_co2_column_test.mSuite.outputPathBase,
                                img_filename_base)
    plt.legend(loc = 'best')
    ...
    plt.clf()
    salt_co2_column_test.mSuite.analysisImages.append(img_filename + '.png')

# generate report:

for rGen in getGenerators(["RST"], salt_co2_column_test.outputPathBase):
    report_filename = os.path.join(salt_co2_column_test.outputPathBase,
                     "%s-report.%s" % (salt_co2_column_test.testName, rGen.stdExt))
    sReps.makeSciBenchReport(salt_co2_column_test, mResults, rGen, report_filename)
    html_filename = os.path.join(salt_co2_column_test.outputPathBase,
                     "%s-report.%s" % (salt_co2_column_test.testName, 'html'))
