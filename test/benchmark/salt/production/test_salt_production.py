...
from credo.systest import FieldWithinTolTC, HistoryWithinTolTC
...
sat_tol = 0.15
field_tols = {"Pressure": 0.01, "Temperature": 0.01, "Liquid saturation": sat_tol,
...
salt_production_test.mSuite.addRun(model_run, run_name)

salt_production_test.setupEmptyTestCompsList()
...
salt_production_test.addTestComp(run_index, "AUTOUGH2 t = 1.e6 s",
        FieldWithinTolTC(fieldsToTest = test_fields,
                         fieldTols = field_tols,
                         expected = AUTOUGH2_result,
                         testOutputIndex = time_index))

jrunner = SimpleJobRunner(mpi = mpi)
testResult, mResults = salt_production_test.runTest(jrunner, createReports = True)
...
for field_name in test_fields:
    ...
    unit = field_unit[field_name] if field_name in field_unit else ""
    result = salt_production_test.mSuite.resultsList[run_index]
    var = result.getFieldAtOutputIndex(field_name, time_index) / scale
    plt.semilogx(r[ir], var[ir], '-', label = 'Waiwera', zorder = 2)
    var = AUTOUGH2_result.getFieldAtOutputIndex(field_name, time_index) / scale
    plt.semilogx(r[ir], var[ir], 's', label = 'AUTOUGH2', zorder = 1)
    plt.xlabel('radius (m)')
    plt.ylabel(field_name + ' (' + unit + ')')
    plt.title(field_name)
    img_filename_base = '_'.join((model_name, run_name, field_name))
    img_filename_base = img_filename_base.replace(' ', '_')
    img_filename = os.path.join(salt_production_test.mSuite.runs[run_index].basePath,
                                salt_production_test.mSuite.outputPathBase,
                                img_filename_base)
    plt.legend(loc = 'best')
    ...
    plt.clf()
    salt_production_test.mSuite.analysisImages.append(img_filename + '.png')

# generate report:

for rGen in getGenerators(["RST"], salt_production_test.outputPathBase):
    report_filename = os.path.join(salt_production_test.outputPathBase,
                     "%s-report.%s" % (salt_production_test.testName, rGen.stdExt))
    sReps.makeSciBenchReport(salt_production_test, mResults, rGen, report_filename)
    html_filename = os.path.join(salt_production_test.outputPathBase,
                     "%s-report.%s" % (salt_production_test.testName, 'html'))
