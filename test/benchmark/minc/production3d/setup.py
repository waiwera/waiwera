...
for case in ['base', 'refined']:
    ...
    model_case_name = model_name + '_' + case
    ...
    else: # locally refined mesh:
        dx = np.array([500]*10)
        dy = np.array([500]*8)
        geo = mulgrid().rectangular(dx, dy, dz, atmos_type = 1, convention = 0,
                                         block_order = 'dmplex')
        refine_rect = [np.array([0, 0]), np.array([1500, 1200])]
        cols = geo.columns_in_polygon(refine_rect)
        geo.refine(cols)
        tri_cols = [col for col in geo.columnlist if col.num_nodes == 3]
        opt_nodes = geo.nodes_in_columns(tri_cols)
        opt_node_names = [n.name for n in opt_nodes if n.pos[0]> 0.1 and n.pos[1] > 0.1]
        geo.optimize(opt_node_names)
        
    geo.write('g' + model_case_name + '.dat')
    mesh_filename = 'g' + model_case_name + '.exo'
    geo.write_mesh(mesh_filename)
    
    dat = t2data()
    dat.title = '3-D MINC production model, %s case' % case
    ...
    dat.write(model_case_name + '_ss.dat')

    inc = dat.grid.incons((P0, T0))
    inc.write(model_case_name + '_ss.incon')
    dat.run(simulator = AUTOUGH2, silent = True)
    inc = t2incon(model_case_name + '_ss.save')

    # production run:

    blk = geo.block_name_containing_point(np.array([1, 1, -1000.]))
    ...
    minc_inc.write(model_case_name + '.incon')
    dat.run(simulator = AUTOUGH2, silent = True)
