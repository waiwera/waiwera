...
Also, meshes may be imported from TOUGH2 models using `PyTOUGH <https://github.com/acroucher/PyTOUGH>`_ (see :ref:`importing`).

.. _gmsh_versions:

GMSH format versions
--------------------

The GMSH format has several different versions. GMSH format version 4 introduced a new section called **Entities**. In general this section is optional, but PETSc's DMPlex currently requires it to be present. Hence it is important when creating meshes in GMSH version 4 or later to ensure that the Entities section is included. This will be the case if the GMSH software is used to create a mesh.

The `meshio <https://pypi.org/project/meshio/>`_ library, which both PyTOUGH and Layermesh use to write GMSH files, does not include the Entities section. Hence, at present, when using these tools to generate a mesh it is best to use an earlier version of the GMSH file format, e.g. 2.2. An example of this can be found in :ref:`importing`.
